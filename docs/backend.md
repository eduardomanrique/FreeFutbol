...
O release de 23/09 usava protocolo1. A versão atual usa protocolo2 para suportar o futevôlei online; frontend e backend precisam ser publicados juntos. O script antigo `activate-vps.sh` atualiza apenas o frontend e não deve ser usado em mudanças de protocolo. `deploy/activate-stack.sh <frontend-id> <backend-id>` verifica os dois pacotes, salva backups, constrói imagens versionadas e atualiza somente os dois serviços, verificando saúde e preservando IDs dos demais containers. Rollback precisa manter versões compatíveis e encerra as partidas em andamento quando reinicia o backend.
...
Futevôlei (implementação de24/09/2026): protocolo2, quatro assentos por sala, IA em vagas/desconexões e worker autoritativo. Snapshots incluem `variant`, `footvolley`, poses e acknowledgements por atleta; futebol mantém dois acknowledgements por equipe. Frontend e backend devem ser publicados juntos.

Em 24/09/2026, frontend `20260924-78513c882edd` e backend `20260924-0dd0da235717` foram publicados juntos, a partir do commit `ebb1669`. Ambos ficaram saudáveis; `https://kmworks.dev/futebol/api/healthz` respondeu `protocol:2`, e a página pública respondeu HTTP 200 com os assets do release. Não havia salas ou partidas ativas antes da troca, e os demais containers mantiveram seus IDs. Backup: `/opt/futebol/deploy-backups/stack-20260924-78513c882edd`. Releases anteriores: frontend `20260923-3676186704ca` e backend `20260923-ca8814c76126`.

Nova publicação da jogabilidade: frontend `20260924-4cfd75a775f4` e backend `20260924-ac8c0da2477c`, commit `7d51596`. Ambos saudáveis, protocolo2, HTML e JavaScript públicos idênticos ao build local. Zero salas/partidas antes da troca; demais containers preservados. Backup: `/opt/futebol/deploy-backups/stack-20260924-4cfd75a775f4`. Versões anteriores: `20260924-78513c882edd` / `20260924-0dd0da235717`.
